#ifndef ROMANIAN_WHIST_COMMON_CARD_MASK_H
#define ROMANIAN_WHIST_COMMON_CARD_MASK_H
...
// Cards as bits, so that "which of these can beat mine?" is an AND and a
// popcount rather than a loop over a vector.
//
// A card's bit is `suit * ranksPerSuit + (rank - minRankInt)`, which lays each
// suit out as a contiguous ascending run - that is what lets maskAbove() and
// maskBelow() be table lookups instead of arithmetic. The whole DeckProfile
// table is built at compile time, once per player count.
//
// Shared, not Reckoner's: every strategy that remembers a round wants this, and
// the deck-relative rank index it computes is also what "big card" means to
// Cyborg. It lived under strategies/reckoner/ until the round tracker was
// extracted; see strategies/reckoner/CardMask.h for the alias that keeps the
// old include path working.
...
inline const DeckProfile& getDeckProfile(unsigned int playerCount)
{
    ...
    {
        throw std::invalid_argument("CardMask: playerCount must be between 2 and 6");
    }
...
inline CardId cardToId(const Card& card, unsigned int playerCount)
{
    ...
    {
        throw std::invalid_argument("CardMask: card rank " + std::to_string(static_cast<int>(card.rank)) +
...
inline Card idToCard(CardId c, unsigned int playerCount)
{
    ...
    {
        throw std::invalid_argument("CardMask: card ID " + std::to_string(c) +
